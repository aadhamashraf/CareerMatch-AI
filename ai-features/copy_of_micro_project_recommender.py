# -*- coding: utf-8 -*-
"""Copy of Micro_Project_Recommender.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16dMZEUddaHrBT4F5ykTtXRNCLTQnWXJ-

# Micro-Project Recommendation Engine (Colab-Ready)

Suggest **2–5 day** micro-projects that help users learn and **prove** missing skills for a target job.
**Inputs (2):**
1) **CV JSON** (upload)
2) **Job Title** (optional: description & essential skills)

**Method**
- Extract present skills from CV, detect **missing skills** (vs. essentials)
- Rank micro-projects per missing skill using **semantic similarity** (SBERT `all-MiniLM-L6-v2`) with **TF-IDF fallback**
- **Rule filters**: duration (2–5 days), optional difficulty and time budget
- Output: Top-k per skill + merged table with links and rationales
"""
...
"""
## 1) Upload CV JSON
On Colab, run and upload `cv_parsed.json`.
"""
...
"""
## 4) Micro-Project Catalog (2–5 day projects)
You can replace this list with your own CSV/DB.  
**Fields:** id, title, source, url, tags, days, difficulty(1–5), description
"""
...
"""
## 5) (Optional) Preferences / Filters
"""

# Limit by difficulty (1–5), None = no filter
PREF_DIFFICULTY_MAX = None  # e.g., 3
# Rough time budget in days (cap results at <= this), None = no filter
PREF_TIME_BUDGET_DAYS = None  # e.g., 3
...
def tfidf_score(query: str, docs: List[str]) -> np.ndarray:
    ...
    if sims.max() > 0: sims = sims / sims.max()  # normalize [0,1]
    return sims
...
def recommend_for_skill(skill: str, df: pd.DataFrame, topk=3, role_hint: str = "") -> pd.DataFrame:
    docs = df["text"].tolist()
    # enrich query with role hint to bias to relevant stack
    query = f"{skill} for {role_hint}" if role_hint else skill
    sims = semantic_scores(query, docs)  # [0..1]
    df = df.copy()
    df["score"] = sims
    df = df.sort_values("score", ascending=False)
    # simple diversity: keep unique first tokens of title
    picks, seen = [], set()
...
def recommend_projects(cv_obj: Dict,
                       essential: List[str],
                       project_df: pd.DataFrame,
                       k_per_skill: int = 3,
                       role_hint: str = "") -> Dict[str, pd.DataFrame]:
