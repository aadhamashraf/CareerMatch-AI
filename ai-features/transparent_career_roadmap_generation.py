# -*- coding: utf-8 -*-
"""Transparent Career Roadmap Generation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1R8kQIse_McTy4D-yc7YD-2UWTwOet4zZ
"""

"""
Transparent Career Roadmap Generation — Clean Prototype
--------------------------------------------------------
Implements:
- Resume parsing (regex + optional spaCy)
- Skill normalization
- Knowledge graph lookup
- Gap analysis
- Roadmap scheduling (with prerequisites)
- Narrative generation
- FastAPI endpoint: /generate_roadmap (POST)

Run:
1. pip install fastapi uvicorn pydantic spacy transformers python-multipart
2. python -m spacy download en_core_web_sm
3. uvicorn transparent_career_roadmap:app --reload --port 8000

This is a prototype — replace toy data and logic with real integrations.
"""
...
# Optional: spaCy for better resume parsing
try:
    import spacy
...
# Toy Knowledge Graph (replace with ESCO / O*NET)
# -----------------------------------------------------
KNOWLEDGE_GRAPH = {
...
def simple_skill_extractor(text: str) -> List[str]:
    """Extract skills from resume text using regex and optional spaCy."""
    text_low = text.lower()
    found = set()
...
def resolve_prereqs(skill: str, role_req: Dict[str, Any]) -> List[str]:
    """Recursively collect prerequisites for a given skill."""
...
def schedule_steps(gaps: List[Dict[str, Any]], role_req: Dict[str, Any], detected: List[str]) -> List[RoadmapStep]:
    """Generate ordered roadmap including prerequisites."""
    roadmap = []
    seen = set()
            ...
            if p not in detected and p not in seen:
                roadmap.append(
                    RoadmapStep(
                        ...
                        resource=role_req.get("resources", {}).get(p),
                    )
                )
                seen.add(p)
        ...
        if skill not in seen:
            roadmap.append(
                RoadmapStep(
                    ...
                    type="Course/Project",
                    duration_weeks=3,
                    reason=f"Core requirement for {role_req}",
                    resource=role_req.get("resources", {}).get(skill),
                )
            )
            seen.add(skill)
...
def generate_narrative(detected: List[str], gaps: List[Dict[str, Any]], role: str) -> str:
    """Generate a natural narrative for the roadmap."""
    if not gaps:
    ...
    return (
        f"For the {role} role, you are missing key skills such as {', '.join(gap_names)}. "
        f"Start by covering their prerequisites, then move to advanced applications. "
        f"Completing these topics will significantly strengthen your fit for {role}."
    )
