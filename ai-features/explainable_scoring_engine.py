# -*- coding: utf-8 -*-
"""Explainable Scoring Engine.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XCZzPvK1RlKzbU52UtKE3VWPeMPDVla7
"""
...
if COLAB:
    ...
    with open("cv_parsed.json", "r", encoding="utf-8") as f:
        cv = json.load(f)
    print("Loaded local cv_parsed.json")
...
essential_skills = []  # leave [] to auto-infer from title
...
scores = {"Strength_S": round(S_detail["S"], 2),
          "Relevance_R": round(R, 2),
          "Completeness_C": round(C, 2)}
subscores = {"Education": round(S_detail["Edu"], 2),
...
print("\n--- Subscores (0–1) ---"); print(subscores)

# ============== 5) Explainable Scoring (Template-based) ==============
...
def make_explanation(scores: Dict[str,float], subs: Dict[str,float], cv_obj: Dict, essentials, job_title: str):
    ...
    if missing:
        next_steps = ("To improve quickly, focus on the missing essentials: " +
                      ", ".join(missing[:8]) +
                      ". Add a short project or course for each one.")
    else:
...
explanation = make_explanation(scores, subscores, cv, essential_skills, job_title)
explanation

# ============== 6) Render Explanation (Markdown) ==============
...
payload_for_llm = {
    ...
    "scores": scores,
    "subscores": subscores,
    "cv_text_excerpt": cv_to_text(cv)[:1500]
}
payload_for_llm  # Pass this into your LLM prompt if desired